...
export default function AdminLogin() {
    ...
    useEffect(() => {
        ...
        if (!token) {
            setStatus("error");
            setErrorMessage("Token bulunamadı. Lütfen geçerli bir bağlantı kullanın.");
        ...
        (async () => {
            try {
                const response = await customerApi.adminLogin(token);
                const authToken = response?.token || response?.data?.token;

                if (!authToken) {
                    throw new Error("Giriş token'ı alınamadı");
                }
                ...
                setStatus("error");
                setErrorMessage(error?.message || error?.data?.message || "Admin girişi başarısız oldu");
                const timer = setTimeout(() => navigate("/login"), 2000);
                return () => clearTimeout(timer);
            }
        })();
                ...
                <AnimatePresence mode="wait">
                    {status === "loading" && (
                        <motion.div
                            ...
                            className="space-y-6"
                        >
                            {/* Admin icon with pulse */}
                            <motion.div
                                animate={{ scale: [1, 1.1, 1] }}
                                transition={{ duration: 1.5, repeat: Infinity }}
                                className="w-20 h-20 mx-auto rounded-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center shadow-lg"
                            >
